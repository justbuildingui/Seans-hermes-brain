---
tags: [personal, fitness, sean-program]
status: starting
started: 2026-04
---

# Ironman Training

> Sean just started training for an Ironman (as of 2026-04).

## State
- **Status:** Just starting
- **Race target:** TBD
- **Training plan:** TBD
- **Current fitness baseline:** TBD

## Open Threads
- Pick a target race and date
- Lock in a training plan / coach
- Baseline tests (swim, bike FTP, run threshold)
- Nutrition and recovery plan

## See Also
- [Sean's user profile](../USER.md)

---
<!-- timeline -->

## 2026-04-20 — Referenced in [concepts/gbrain](../concepts/gbrain.md)
Listed as a "See Also" from the GBrain concept page (Sean's active programs context). Source: GBrain install conversation with Ollie.

## 2026-04-20 — program created
Sean mentioned he's starting Ironman training during Ollie soul-audit. Source: soul-audit conversation.
